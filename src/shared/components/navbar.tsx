...
export function NavbarAdmin() {
  ...
  const navList = navInfo.map((info, index) => (
    <li key={index} className="w-full">
      ...
      >
        <FontAwesomeIcon icon={info.icon} className="me-2 text-cyan-700" style={{width: '16px', height: '16px'}}/>
  ...
  return (
    <div className="">
        ...
        <nav className="p-6 flex-1">
          <ul className="space-y-4 px-0">
            ...
            <div
                ...
                onClick={() => logout()}
              >
                <FontAwesomeIcon icon={faSignOutAlt} className="me-2 text-cyan-700" style={{width: '16px', height: '16px'}}/>
                <p className="text-cyan-700 mb-0">
                  Logout
                </p>
            </div>
      ...
      <div className="flex md:hidden items-center justify-between bg-white border-b border-gray-300 px-4 py-3 fixed top-0 left-0 right-0 z-30">
        <Link href="/pacientes">
          <h1 className="text-xl font-bold text-cyan-700">Admin</h1>
        </Link>
        <button
          className="p-2 rounded focus:outline-none focus:ring-2 text-cyan-700 focus:ring-cyan-500"
          onClick={() => setOpen((o) => !o)}
