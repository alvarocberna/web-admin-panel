...
`NEXT_PUBLIC_BACKEND_URL` se pasa como placeholder (`http://localhost:3001` si el secret no está definido) porque el build solo verifica que el proyecto compile — no hay runtime real en este job.
...
| Secret | Uso |
|--------|-----|
| `NEXT_PUBLIC_BACKEND_URL` | URL del backend real contra el que corren los tests E2E |
